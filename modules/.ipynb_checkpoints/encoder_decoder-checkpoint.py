...
def get_mask(q_embeddings, tgt_embeddings, tgt_mask):
    question_length = q_embeddings.shape[1]
    tgt_length = tgt_embeddings.shape[1]
    tl = question_length + tgt_length

    masks = torch.full((tl,tl), True)
    masks[question_length:,question_length:] = tgt_mask
    masks[:question_length,question_length:] = False

    return masks.unsqueeze(0).to(tgt_mask)
...
class Transformer(nn.Module):
    ...
    def forward(self, src, tgt, src_mask, tgt_mask,q_embeddings):
        return self.decode(self.encode(src, src_mask), src_mask, tgt, tgt_mask, q_embeddings)

    def encode(self, src, src_mask):
        return self.encoder(self.src_embed(src), src_mask)

    def decode(self, hidden_states, src_mask, tgt, tgt_mask, q_embeddings):
        tgt_embeddings = self.tgt_embed(tgt)
        tgt_mask = get_mask(q_embeddings, tgt_embeddings, tgt_mask)
        return self.decoder(torch.cat([q_embeddings.repeat(tgt.shape[0],1,1),tgt_embeddings],dim=1), hidden_states, src_mask, tgt_mask)
...
class EncoderDecoder(AttModel):
    ...
    def _prepare_feature(self, att_feats, att_masks, meshes= None):
    ...
    def _forward(self, att_feats,  q_embeddings,answer_ids, att_masks=None):
        att_feats, answer_ids, att_masks, answer_mask = self._prepare_feature_mesh(att_feats, att_masks, answer_ids)
        out = self.model(att_feats,answer_ids, att_masks, answer_mask, q_embeddings)
    ...
    def core(self, it, memory, state, mask, q_feats):
        ...
        out = self.model.decode(memory, mask, ys, subsequent_mask(ys.size(1)).to(memory.device),q_feats)
