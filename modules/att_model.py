...
class AttModel(CaptionModel):
    ...
    def _sample_beam(self, att_feats, q_feats,att_masks=None, meshes=None, opt={}):
        ...
        logprobs, state = self.get_logprobs_state(it, pp_att_feats, p_att_masks, q_feats, state)

        pp_att_feats, p_att_masks = utils.repeat_tensors(beam_size,[pp_att_feats, p_att_masks])
        self.done_beams = self.beam_search(state, logprobs,  pp_att_feats, p_att_masks, q_feats,opt=opt)
    ...
    def _sample(self, att_feats, q_feats, meshes=None, att_masks=None):
        ...
        if beam_size > 1 and sample_method in ['greedy', 'beam_search']:
            return self._sample_beam(att_feats, q_feats,att_masks, meshes, opt)
        if group_size > 1:
        ...
        pp_att_feats, p_att_masks = self._prepare_feature(att_feats, att_masks, meshes)
